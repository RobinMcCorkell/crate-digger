...
This is followed by _seq~page~_ which is a sequence number updated to the global next value every time any page is edited (see also: _seq~db~_ in the database header). When the page is edited, this value is set to the value of _seq~db~_ and then _seq~db~_ is incremented. Sequence numbers of different pages can be used to determine which pages were touched in which order during an export.
...
Byte{nbsp}``1b`` is called __page_flags__ (abbreviated _p~f~_ in the diagram) which is partially understood:
...
|7 |_u~7~_        |Unknown, always unset?
|6 |_I_ (index)   |Page contains index entries rather than data entries.
|5 |_u~5~_        |Unknown, always set?
|4 |_D_ (deleted) |Data page contains deleted/invalid rows.
|3 |_u~3~_        |Unknown, always unset?
|2 |_u~2~_        |Unknown, always set?
|1 |_u~1~_        |Unknown, always unset?
|0 |_u~0~_        |Unknown, always unset?
|===

Index pages typically always have a flags value of `0x64`, while data pages typically have a flags value of `0x24` or `0x34`.
Crate Digger considers a page to be a data page if __page_flags__&``0x40``{nbsp}={nbsp}`0`.

Bytes{nbsp}``1c``-`1d` are called __free_size__ (abbreviated _free~s~_ in the diagram), and store the amount of unused space in the page heap (excluding the row index which is built backwards from the end of the page); __used_size__ at bytes{nbsp}``1e``-`1f` (abbreviated _used~s~_) stores the number of bytes that are in use in the page heap.
...
Bytes{nbsp}``22``-`23` are called __transaction_row_index__ (abbreviated _tran~ri~_ in the diagram), and store the index of the first row touched in the last transaction on this page.

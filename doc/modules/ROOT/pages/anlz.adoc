...
The value zero means the user has made no assignment, and this is treated the same as if “Cool” has been chosen.
The values and their meanings are listed in the <<track-banks,table below>>.
We have recently seen unexpected values here, such as `f6` and `f9`, so the Kaitai Struct mapping has split the modeling of this byte to a `raw_bank` integer, which will always have a value, and the `bank` enum has become a value instance that can be `null` for unrecognized values, but will not prevent the overall tag from being parsed and constructed.
